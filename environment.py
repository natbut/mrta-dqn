...
class Environment:
    def __init__(self, num_agents, tasks_vs_reqd_agents, tasks_vs_reqd_time, task_to_task_transition_times, starting_task_id = 0) -> None:
        '''
        ...
        self.env_var_agents_time_to_complete =                [0 for agent in range(self.env_con_num_agents)]
        self.env_var_agents_time_to_reach    =                [0 for agent in range(self.env_con_num_agents)]
        self.env_var_agents_cum_idle_time    =                [0 for agent in range(self.env_con_num_agents)]
        self.env_var_agents_prev_task        = [starting_task_id for agent in range(self.env_con_num_agents)]
        self.env_var_tasks_completed         =  [0 for task in range(len(self.env_con_tasks_vs_reqd_agents))]
        self.env_var_tasks_time_left         =                                      tasks_vs_reqd_time.copy()

        # initialise the decision variables as numpy matrices
        self.dec_var_agents_at_tasks             = np.zeros((num_agents, len(self.env_con_tasks_vs_reqd_agents)), 
                                                             np.bool8) # maps which task each agent is associated with
        self.dec_var_agents_current_action       = np.zeros((num_agents, 3), 
                                                             np.bool8) # maps which action each agent is currently performing
        
        self.dec_var_agents_at_tasks[:, self.env_con_starting_task_id] = 1 # initialise all agents to starting task id
    ...
    def step(self, agents_vs_actions):
        ...
        for agent_action, i in zip(agents_vs_actions, range(len(agents_vs_actions))):
            ...
            if self.dec_var_agents_current_action[i][APPROACHING] == 1 and self.env_var_agents_time_to_reach[i] > 0:
                self.env_var_agents_time_to_reach[i] -= 1
                continue
            # if agent is in 'work' with >0 time left, it cannot change actions
            elif self.dec_var_agents_current_action[i][WORKING] == 1 and self.env_var_agents_time_to_complete[i] > 0:
                reward[i] = 1
                continue
            ...
            if agent_action[1] == WORKING:
                ...
                self.env_var_agents_time_to_complete[i] = self.env_var_tasks_time_left[agent_action[0]]
                reward[i] = 1
                ...
                self.env_var_agents_prev_task[i] = np.argmax(self.dec_var_agents_at_tasks[i]) # where the agent was in the previous timestep
                self.env_var_agents_time_to_reach[i] = self.env_con_task_to_task_transition_times[self.env_var_agents_prev_task[i]][agent_action[0]] # initiliase as the transition time
                self.env_var_agents_time_to_complete[i] = 0
            ...
            self.dec_var_agents_at_tasks[i][agent_action[0]] = True # updating the task id
            self.dec_var_agents_current_action[i][agent_action[1]] = True # update the assignment
        ...
        for task_id in range(len(self.dec_var_agents_at_tasks[0])):
            arr_working_agents = self.dec_var_agents_at_tasks[:, task_id] & self.dec_var_agents_current_action[:, WORKING]
            ...
            if np.sum(arr_working_agents) >= self.env_con_tasks_vs_reqd_agents[task_id] and self.env_var_tasks_time_left[task_id] > 0:
                self.env_var_tasks_time_left[task_id] -= 1
                for agent_i in range(len(self.dec_var_agents_at_tasks[:, task_id])):
                    if arr_working_agents[agent_i] == True: # if agent_1 is working at that task
...
class TaskVisualization:
    ...
    def _display_agents(self, agent_vs_task_mat, dot_colour='black', action=APPROACHING):
                ...
                for task_id in range(agent_vs_task_mat.shape[1]):
                    if agent_vs_task_mat[agent_id, task_id]:
                        if action == WORKING or action == IDLE:
                            ...
                            prev_task_x_pos, prev_task_y_pos = self.pos[env.env_var_agents_prev_task[agent_id]]
                            x_line , y_line  = next_task_x_pos - prev_task_x_pos, next_task_y_pos - prev_task_y_pos
                            t_steps = env.env_con_task_to_task_transition_times[task_id][env.env_var_agents_prev_task[agent_id]]
                            x_delta, y_delta = x_line/t_steps, y_line/t_steps
                            t_steps_left = env.env_var_agents_time_to_reach[agent_id]
                            x_pos, y_pos = prev_task_x_pos + x_delta*(t_steps - t_steps_left), prev_task_y_pos + y_delta*(t_steps - t_steps_left)
    ...
    def update(self, env):
        ...
        working_agents_vs_tasks     = np.mat([env.dec_var_agents_at_tasks[:, col_id] & env.dec_var_agents_current_action[:, WORKING]     for col_id in range(env.dec_var_agents_at_tasks.shape[1])]).T
        approaching_agents_vs_tasks = np.mat([env.dec_var_agents_at_tasks[:, col_id] & env.dec_var_agents_current_action[:, APPROACHING] for col_id in range(env.dec_var_agents_at_tasks.shape[1])]).T
        idle_agents_vs_tasks        = np.mat([env.dec_var_agents_at_tasks[:, col_id] & env.dec_var_agents_current_action[:, IDLE]        for col_id in range(env.dec_var_agents_at_tasks.shape[1])]).T
        ...
        nx.draw_networkx_nodes(self.G, self.pos, node_size=1000, node_color='skyblue', node_shape='o')
        ...
        self._display_agents(working_agents_vs_tasks,     dot_colour='green', action=WORKING    )
        self._display_agents(approaching_agents_vs_tasks, dot_colour='blue',  action=APPROACHING)
...
if __name__ == "__main__":
    ...
    actions_list = [
        [(1, APPROACHING), (2, APPROACHING), (2, APPROACHING), (3, APPROACHING), (0, IDLE)],
        [(1, APPROACHING), (2, APPROACHING), (2, APPROACHING), (3, APPROACHING), (0, IDLE)],
        [(1, APPROACHING), (2, APPROACHING), (2, APPROACHING), (3, APPROACHING), (0, IDLE)],
